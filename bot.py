...
def _get_chat_id(name: str):
    ...
    try:
        ...
        raise ValueError(f"Environment variable {name} must be an integer chat id")

LEADS_CHAT_ID = _get_chat_id("LEADS_CHAT_ID")   # например: -1001234567890

# 4) (необязательно) Ваш личный Telegram ID, чтобы получать уведомления о заявках.
ADMIN_CHAT_ID = _get_chat_id("ADMIN_CHAT_ID")
...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("✅ Бот отвечает")
    if ADMIN_CHAT_ID:
        await send_notification(context, "🔔 Тестовое уведомление от бота: бот отвечает")
...
async def send_notification(context, text: str):
    recipients = []
    if LEADS_CHAT_ID is not None:
        recipients.append(LEADS_CHAT_ID)
    if ADMIN_CHAT_ID is not None:
        recipients.append(ADMIN_CHAT_ID)

    if not recipients:
        logging.info("Уведомление не отправлено: ни ADMIN_CHAT_ID, ни LEADS_CHAT_ID не заданы")
        return
    ...
    for chat_id in recipients:
        try:
            ...
            logging.error(f"Не удалось отправить уведомление в {chat_id}: {e}")
...
async def send_lead_to_owner(context, name, phone, kind, extra, username):
    ...
    await send_notification(context, lead)
...
def main():
    ...
    print(f"✅ Бот запущен. Откройте его в Telegram и нажмите /start")
    if ADMIN_CHAT_ID is not None:
        print(f"✅ ADMIN_CHAT_ID = {ADMIN_CHAT_ID}")
    if LEADS_CHAT_ID is not None:
        print(f"✅ LEADS_CHAT_ID = {LEADS_CHAT_ID}")
