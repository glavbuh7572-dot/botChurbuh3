...
# 4) (необязательно) Ваш личный Telegram ID, чтобы получать уведомления о запуске и заявках.
#    Можно задать через переменную окружения ADMIN_CHAT_ID.
...
async def send_start_notification(context: ContextTypes.DEFAULT_TYPE):
    if not ADMIN_CHAT_ID and not LEADS_CHAT_ID:
        return
    message = "✅ Бот запущен и готов принимать сообщения"
...
def main():
    ...
    app.job_queue.run_once(send_start_notification, when=2)
